...
describe("getComparableNotes", () => {
  ...
  it("should not treat a list with content as empty", () => {
    expect(getComparableNotes(ScrapType.List, asNotes(item("milk")))).not.toBe(
      getComparableNotes(ScrapType.List, "[]"),
    );
  });
...
describe("hasSomethingToSave", () => {
  it("should not save a new list holding only a blank item", () => {
    expect(
      hasSomethingToSave(ScrapType.List, asNotes(item("")), "", true),
    ).toBe(false);
  });

  it("should not save a new, entirely untouched scrap", () => {
    expect(hasSomethingToSave(ScrapType.List, undefined, "", true)).toBe(false);
  });

  it("should save a new list holding a blank item when there is a title", () => {
    expect(
      hasSomethingToSave(ScrapType.List, asNotes(item("")), "groceries", true),
    ).toBe(true);
  });

  it("should save a new list that has content", () => {
    expect(
      hasSomethingToSave(ScrapType.List, asNotes(item("milk")), "", true),
    ).toBe(true);
  });

  // The user may have just cleared the last item, and that has to reach the server.
  it("should save an existing list that was emptied", () => {
    expect(hasSomethingToSave(ScrapType.List, "[]", "", false)).toBe(true);
  });

  it("should not save an existing scrap with neither notes nor title", () => {
    expect(hasSomethingToSave(ScrapType.List, undefined, "", false)).toBe(
      false,
    );
  });
});
