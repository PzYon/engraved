...
public class MoveEntryCommandExecutorShould
{
  ...
  public async Task MoveEntryToOtherJournal()
  {
    ...
    IEntry[] sourceEntries = await _repo.GetEntriesForJournal("source-journal-id", null, null, null, null);
    sourceEntries.Length.Should().Be(1);

    // given: target
    _repo.Journals.Add(new CounterJournal { Id = "target-journal-id" });
    IEntry[] targetEntries = await _repo.GetEntriesForJournal("target-journal-id", null, null, null, null);
    ...
    // then
    targetEntries = await _repo.GetEntriesForJournal("target-journal-id", null, null, null, null);
    targetEntries.Length.Should().Be(1);

    sourceEntries = await _repo.GetEntriesForJournal("source-journal-id", null, null, null, null);
