...
// Resolves to the raw MongoRepository. Inject this only where there is deliberately no current user
// to scope to: the notification job (runs across all users), authentication/login (runs before a
// user context exists) and health endpoints.
//
...
public interface IUnrestrictedRepository : IRepository
{
}
